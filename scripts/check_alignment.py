...
def get_gitignore_patterns(project_path: Path) -> Tuple[str, ...]:
    ...
    with gitignore_path.open("r", encoding="utf-8",
                             errors="ignore") as gitignore_file:
        return tuple(line.strip() for line in gitignore_file
...
def is_ignored(file_path: str, gitignore_patterns: Tuple[str, ...]) -> bool:
    ...
    return any(
        fnmatch.fnmatch(file_path, pattern) for pattern in gitignore_patterns)
...
def is_sensitive_file(file_path: str) -> bool:
    ...
    return any(pattern in Path(file_path).name.lower()
...
def get_relevant_files(project_path: Path) -> List[Tuple[Path, int]]:
    ...
    for root, dirs, files in os.walk(project_path):
        ...
        for file in files:
            ...
            if (not is_ignored(str(file_path), gitignore_patterns)
                ...
                if file_path.suffix in priority_extensions:
                    ...
                    priority = (len(priority_extensions) +
                                priority_names.index(file_path.stem) + 1)
                else:
                    priority = len(priority_extensions) + len(
                        priority_names) + 1
...
def generate_archive(project_path: Path,
                     relevant_files: List[Tuple[Path, int]]) -> str:
        ...
        messages=[
            {
                ...
                "system",
                "content":
                "You are an AI assistant tasked with analyzing project structure and alignment between README and Makefile.",
            },
            {
                "role": "user",
                "content": prompt
...
def generate_patch(original_content: str, updated_content: str) -> str:
    ...
    diff = difflib.unified_diff(original_lines,
                                updated_lines,
                                fromfile="original",
                                tofile="updated",
                                n=3)
...
def create_empty_patch_file(project_path: Path) -> str:
    ...
    with patch_file_path.open("w") as f:
        ...
        f.write(
            f"# You can apply this patch using: patch -p0 < {patch_file_name}\n"
        )
...
@click.option("--install-dependencies",
              is_flag=True,
              help="Install missing dependencies")
@click.option("--create-empty-patch",
              is_flag=True,
              help="Create an empty patch file")
@click.option("--batch-mode",
              is_flag=True,
              help="Run in batch mode for multiple projects")
def main(
    project_path: str,
    use_llm: str,
    ollama_model: str,
    ...
    try:
        ...
        if use_llm:
            click.echo(
                f"Sending prompt to {use_llm.capitalize()} for analysis...")
            try:
                if use_llm == "claude":
                ...
                updated_readme, updated_makefile = parse_llm_response(
                    llm_response)

                readme_patch = generate_patch(original_readme, updated_readme)
                makefile_patch = generate_patch(original_makefile,
                                                updated_makefile)

                patch_output_path = (project_path.parent /
                                     f"{project_name}_{use_llm}_patch.diff")
                with patch_output_path.open("w",
        ...
        click.echo(f"An error occurred during project analysis: {str(e)}",
                   err=True)
