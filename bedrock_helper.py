...
def enrich_node_info(
    node_name: str, attributes: Dict, connections: List[str]
) -> EnrichedNodeInfo:
    try:
...
def query_knowledge_base(
    query: str, max_results: int = 5, max_retries: int = 3, base_delay: float = 1.0
) -> List[KnowledgeBaseResult]:
    knowledgeBaseId = (
        "aif-c01-knowledge-base-quick-start-4uy41"  # Ensure this is correct
    )
    ...
    for attempt in range(max_retries):
        try:
            ...
            logging.info(
                f"Successfully queried knowledge base. Results: {len(response['retrievalResults'])}"
            )
            ...
            for i, result in enumerate(response["retrievalResults"]):
                ...
                if len(parts) >= 3:
                    results.append(
                        KnowledgeBaseResult(
                            relation=parts[1], source=parts[0], target=parts[2], key=i
                        )
                    )
            ...
            if "ValidationException" in str(e):
                logging.error(
                    f"Validation error details: {e.response['Error']['Message']}"
                )
...
if __name__ == "__main__":
    ...
    anthropic_response = invoke_bedrock_model(
        "anthropic.claude-v2", "What is the capital of France?"
    )
    logging.info(f"Anthropic model response: {anthropic_response}")

    ai21_response = invoke_bedrock_model(
        "ai21.j2-ultra-v1", "What is the capital of Germany?"
    )
    logging.info(f"AI21 model response: {ai21_response}")

    amazon_response = invoke_bedrock_model(
        "amazon.titan-text-express-v1", "What is the capital of Italy?"
    )
    ...
    enriched_info = enrich_node_info(
        "Descartes", descartes_attributes, descartes_connections
    )
    logging.info(
        f"Enriched information for Descartes:\n{enriched_info.enriched_content}"
    )
